...
def test_solution(day, only_sample=False, year=None):
    ...
    input_data = read_to_list(f"puzzle_input/{year}/day{day}.txt")
    expected_output = read_to_list(f"expected_output/{year}/day{day}.txt", remove=True)
    ...
    for i, (input, output) in enumerate(zip(input_data, expected_output)):
        ...
        timer = end - start
        timer_seconds = timer / 1_000_000
        if output != "" and output is not None:
            if int(actual_output) == int(output):
                print(
                    f"| ✔ | Output: {actual_output: <10} \t\t| Time: {timer:<8} Nanoseconds, in ms: {timer_seconds} "
                )
            else:
                print(
                    f"| ✗ | Output: {actual_output:<8} != {output:<8} \t| Time: {timer:<8} Nanoseconds, in ms: {timer_seconds}"
                )
        else:
